...
hook.Add("OnParkour", "ParkourXP", function(event)
    ...
    if (ParkourXP_RNG[event] or 1) > math.random() and (not ParkourXP_PosCheck[event] or parkourevent_lastpos:Distance(pos) > 200) then
...
local function SaveXP()
    ...
    if xpold then
        ...
        if util.JSONToTable(xpold)[1] > LocalPlayer().XP then return end
    end
