...
hook.Add("HUDShouldDraw", "BeatrunHUDHide", function(name) if hide[name] then return false end end)
local color = 1
local tab = {
    ["$pp_colour_addr"] = 0,
    ["$pp_colour_addg"] = 0,
...
local function BeatrunHUD()
    ...
    if customname then
        ...
        coursename = (Course_Name ~= "" and Course_Name) or "Freeplay"
    end
    ...
    local bgpadw, bgpadh = nickw, nickh
    if bgpadw < coursew then bgpadw = coursew end
    ...
    if not hidden:GetBool() then
        ...
        for k, v in pairs(XP_floatingxp) do
            ...
            surface.DrawText(v)
            if CurTime() > k then XP_floatingxp[k] = nil end
        end
    end
...
local function sortleaderboard(a, b)
    ...
    if GetGlobalBool(GM_INFECTION) then
        ...
        if btime == 0 then btime = -1 end
        return atime > btime
    else
        if atime == 0 then atime = 1 / 0 end
        if btime == 0 then btime = 1 / 0 end
        return atime < btime
    end
end

function BeatrunLeaderboard(forced)
    if not forced and Course_Name == "" and not GetGlobalBool(GM_INFECTION) then return end
    ...
    for k, v in ipairs(allply) do
        ...
        if pbtimenum ~= 0 then pbtime = string.FormattedTime(v:GetNW2Float("PBTime"), "%02i:%02i:%02i") end
...
hook.Add("RenderScreenspaceEffects", "FilterPP", function() if not blinded or not blindinverted then DrawColorModify(pp) end end)
...
local function DrawSpeedGraph()
    ...
    local offsetx, offsety = ScrW() * sgoffsetx, ScrH() * sgoffsety
    local boxscaledx, boxscaledy = SScaleX(400), SScaleY(200)
...
local function RecordSpeedGraph()
    ...
    if LocalPlayer():GetClimbing() ~= 0 then oldrecord = speedrecord[lenrecord] or 0 end
    if lenrecord >= maxentries then table.remove(speedrecord, 1) end
    if not oldrecord then
        ...
        table.insert(speedrecord, SScaleY(power * (0.001 * (power * 0.000005))))
    else
